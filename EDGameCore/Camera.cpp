...
namespace EDGameCore {
	...
	const Float4x4& Camera::GetViewMatrix(void) {
		...
		Float4x4 tmpVM = this->GetGameObject()->GetTransform()->GetWorldMatrix();
		viewMatrix = tmpVM;
		viewMatrix.Wx = -DotProduct(tmpVM.WAxis, tmpVM.XAxis);
		viewMatrix.Wy = -DotProduct(tmpVM.WAxis, tmpVM.YAxis);
		viewMatrix.Wz = -DotProduct(tmpVM.WAxis, tmpVM.ZAxis);
