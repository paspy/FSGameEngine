...
void MouseLookController::OnMessage(EDGameCore::Message& message, const void * data) {
	...
	if ( message.messageId == MSG_MSG_MOUSE_BEGIN_ROTATE ) {
		...
		float deltaX = float(new_point.x - cursorPos.x) * MOUSE_LOOK_SPEED_MODIFIER;
		float deltaY = float(new_point.y - cursorPos.y) * MOUSE_LOOK_SPEED_MODIFIER;

		Float4x4 rotX, rotY;
		rotX.makeIdentity();
		rotY.makeIdentity();
		rotX.makeRotationX(deltaY);
		rotY.makeRotationY(deltaX);
		camera_mat = rotY  * rotX * camera_mat;

		//camera_mat = camera_mat.rotateLocalY(deltaX);
		//camera_mat = camera_mat.rotateLocalX(deltaY);
		//
		Float3 xAxis = ZERO_VECTOR;
		Float3 yAxis = ZERO_VECTOR;
		Float3 zAxis = camera_mat.ZAxis.normalize();
		...
		Float4x4 localMatrix = Float4x4(
			xAxis.x, xAxis.y, xAxis.z, camera_mat.padX,
			yAxis.x, yAxis.y, yAxis.z, camera_mat.padY,
			zAxis.x, zAxis.y, zAxis.z, camera_mat.padZ,
			camera_mat.WAxis.x, camera_mat.WAxis.y, camera_mat.WAxis.z, camera_mat.padW
		);
		
		GetGameObject()->GetTransform()->SetLocalMatrix(localMatrix);
